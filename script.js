var citations = [
    ...
    ["Le meilleur moyen d’obtenir de l’argent est d’en avoir besoin.", "Mickael Jordan"],
...
function afficherCitation() {
    do {
        indices = genererNombreEntier(citations.length);

    } while (indices === dernier);


    var newCitation = citations[indices][0];
    var newAuteur = citations[indices][1];

    document.getElementById('citation').textContent = newCitation;
    document.getElementById('auteur').textContent = newAuteur;
    dernier = indices;
}


document.getElementById('nouveau').addEventListener('click', afficherCitation);
